...
export function ModuleRow({ code, title, desc, cta, defaultOpen = false }: ModuleRowProps) {
  return (
    <details className="mb-3 rounded-lg border border-white/[0.1] bg-white/[0.02] overflow-hidden" open={defaultOpen}>
      ...
      <div className="px-4 pb-4 pl-8 md:pl-10 border-t border-white/[0.08]">
        ...
        <a
          ...
          className="mt-3.5 inline-block rounded-md border border-white/15 px-3.5 py-2.5 font-mono text-[0.8rem] tracking-[0.12em] text-[hsl(var(--text))] no-underline transition-colors duration-150 hover:border-white/70 hover:text-white hover:bg-white/[0.05]"
        >
          {cta.label}
        </a>
      </div>
